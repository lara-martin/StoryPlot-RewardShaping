...
def create_model(session, forward_only):
  ...
  ckpt = tf.train.get_checkpoint_state(FLAGS.train_dir,"checkpoint_full2")
  print("CKPT is:",ckpt)
...
def train():
  ...
  with tf.Session() as sess:
    ...
    vocab_file = open('Final_genre68_reduced.vocab', 'rb')
    ...
    while epochs < FLAGS.max_epochs:
      ...
      if epochs%500 == 0 and pos<FLAGS.batch_size:
        # Save checkpoint and zero timer and loss.
        # checkpoint_path = os.path.join(FLAGS.train_dir, "translate.ckpt")
        checkpoint_path = os.path.join("/home/twister/Desktop/GALE_RL/Storytelling_Run_MN/storytelling_dl/Neural-Net-Experiments/Spring2017-Attempt/NewEvents-PolicyGradient/checkpoints","forward_run3_epoch{}.ckpt".format(epochs))
        model.saver.save(sess, checkpoint_path, global_step=model.global_step)
        print ("MODEL SAVED")
...
def decode():
  ...
  with tf.Session() as sess:
    ...
    while sentence:
      ...
      token_ids = [int(vocab.index(word)) for word in sentence.split(',')]
      ...
      print(outputs.shape)
      print(",".join([inv_vocab[o[0]] for index,o in enumerate(outputs)]))
