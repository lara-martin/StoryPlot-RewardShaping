...
def get_model_corpus_probability_stats(model_ckpt, input_event_file, vocab_file='Final_genre68_full.vocab', log_file_name='genre_68_eval_stats_logs'):
  ...
  with tf.Session(config = cfg) as sess:
    ...
    with open(input_event_file, "rb") as in_file:
      for event in in_file:
        ...
        # First do the calculation for the forward model - pass the correct model into the function
          ...
          # print(sess, event, model, max_count, samples, placeholder)
          # print(decode_event)
          got_admire, count_val, complete_story = decode_event_story(sess, event, model, max_count, vocab, inv_vocab, samples, placeholder)
          # print(posStats[0][positionDict[0]['<NE>0']][positionDict[0]['this']])
          # exit(0)
          # print(event)
          event = event.replace("\n","")
          event = event.split(',')
          # print(event)
          if len(complete_story)!=0:
            story_line_count = 0
            num = 0
            sum_probs = 0
            for line in complete_story:
                story_line_count+=1
                if num!=0:
                  for p in range(4):
                    if (inv_vocab[prev_line[p]] not in positionDict[p] or inv_vocab[line[p]] not in positionDict[p]):
                      sum_probs+=0
                    else:
                      sum_probs += posStats[p][positionDict[p][inv_vocab[line[p]]]][positionDict[p][inv_vocab[prev_line[p]]]]
                else:
                  for p in range(4):
                    # event = event.split(',')
                    # print(event)
                    if (event[p] not in positionDict[p] or inv_vocab[line[p]] not in positionDict[p]):
                      sum_probs+=0
                    else:
                      sum_probs += posStats[p][positionDict[p][inv_vocab[line[p]]]][positionDict[p][event[p]]]
                    #write_line += str(inv_vocab[line[p]]) + " "
                  #write_line += "\n"
                #exit(0)
                num+=1
                prev_line = line
            # print(float(sum_probs)/story_line_count)
            probabilities.append(float(sum_probs)/story_line_count)
            # print(probabilities)
          sum_probs =0
          if len(complete_story)!=0:
            ll= []
            ...
            for line in complete_story:
              for t in line: 
                ll.append(inv_vocab[t])
                write_line += str(inv_vocab[t]) + " "
              write_line += "\n"
              #log_file.write(write_line)
              lines.append(ll)
              ll = []
            log_file.write(write_line)
          if got_admire and len(complete_story)!=0:
            got_admire_count+=1
            to_admire_lines+=count_val
            num = 0
            # print(posStats[0])
            ...
            #   if num!=0:
            #     for p in range(4):
            #       # print(posStats[p][positionDict[p][inv_vocab[line[p]]]])
            #       # print(inv_vocab[line[p]])
            #       # exit(0)
            #       # print(p)
            #       # print(inv_vocab[line[p]])
            #       # print(inv_vocab[prev_line[p]])
            #       if (inv_vocab[prev_line[p]] not in positionDict[p] or inv_vocab[line[p]] not in positionDict[p]):
            #         sum_probs+=0
            #       else:
            #         sum_probs += posStats[p][positionDict[p][inv_vocab[line[p]]]][positionDict[p][inv_vocab[prev_line[p]]]]
            ...
            # print(event)
            # print(sum_probs)
            # print(long(sum_probs))
            # exit(0)
            # exit(0)
            # probabilities.append(long(sum_probs)/story_line_count)
    ...
    sess.close()
    # print(probabilities)
    print("Average Probability: " + str(sum(probabilities)/len(probabilities)))
    write_line ="Average Probability: " + str(sum(probabilities)/len(probabilities))
    # log_file.write(write_line)
    # log_file.close()
    # Write the results before returning the values
    write_line += "For the model - "+str(model_ckpt)+". The results are:\n"
    write_line += "Got count:"+str(got_admire_count)+"\nTo admire lines:"+str(to_admire_lines)+"\nTotal lines:"+str(num_lines)+"\n"
    write_line += "Average count to admire: " + str(float(to_admire_lines)/got_admire_count)
    write_line += "\nAverage percentage of getting to admire: " + str(long(got_admire_count)/num_lines)
    log_file.write(write_line)
    log_file.close()
    #Percentage of lines that get to admire. (got_admire )
...
def get_corpus_probability_stats(model_ckpt, input_event_file, vocab_file='Final_genre68_full.vocab', log_file_name='genre_68_eval_stats_logs'):
  word_dict_statistics = load_obj('words_for_sampling_parsed_editted')
  posStats,individualPosStats,secondPositionStats,thirdPositionStats,positionDict = statistics(word_dict_statistics,'final_combined_genre68_full.txt')
  horizonPosStats, secondHorizonPosStats = statistics_horizontal(word_dict_statistics, 'final_combined_genre68_full.txt')  
  cfg = tf.ConfigProto()
          ...
          if '<end' in event:
            if len(complete_story)!=0:
              ...
              for line in complete_story:
                ...
                if num!=0:
                  for p in range(4):
                    if (prev_line[p] not in positionDict[p] or line[p] not in positionDict[p]):
                      sum_probs+=0
                    else:
                      sum_probs += posStats[p][positionDict[p][line[p]]][positionDict[p][prev_line[p]]]
                # else:
                #   for p in range(4):
                #     # event = event.split(',')
                #     # print(event)
...
def decode_event_story(sess,cur_event,model,cur_max,vocab,inv_vocab,samples,placeholder):
  """Create translation model and initialize or load parameters in session."""
  lines_generated = 0
  story_flag = False
  cur_event=cur_event.strip('\n')
  ...
  while lines_generated <= cur_max:
    ...
    tt = []
    # for t in token_ids:
    #   tt.append(inv_vocab[t])
    # print(tt)
    complete_story.append(token_ids[:])
    # print(complete_story)
    lines_generated+=1
  # print(complete_story)
  # exit(0)
  # print("after")
  return story_flag,lines_generated,complete_story
...
def main(_):
  ...
  get_model_corpus_probability_stats("checkpoint_forward_genre68_full_run_1","final_test_genre68_full.txt",log_file_name='Run-Logs/Forward_10lines_Probability_Stats')
   #get_corpus_probability_stats("checkpoint_forward_genre68_full_run_1","final_combined_genre68_full.txt",log_file_name='Run-Logs/Corpus_Full_Probability_Stats')
