...
import seq2seq_model_fb_genre68_full as seq2seq_model
import pickle
...
tf.app.flags.DEFINE_string("train_dir", "/home/twister/Desktop/GALE_RL/Storytelling_Run_MN/storytelling_dl/Neural-Net-Experiments/Spring2017-Attempt/NewEvents-PolicyGradient/Full_Genre_68/checkpoints", "Training directory.")
...
def create_model(session, forward_only, filename = "start_fresh"):
...
def train():
  ...
  with tf.Session() as sess:
    ...
    while epochs < FLAGS.max_epochs:
      ...
      # Try doing the reading and check the bucket id, that will be the bucket_id that we will hardcode here
      ...
      if (pos + FLAGS.batch_size) >= len(train_set[bucket_id]):
        ...
        # checkpoint_path = os.path.join(FLAGS.train_dir, ("RL_run1_epoch.ckpt"))
        # checkpoint_path = os.path.join("/home/twister/Desktop/GALE_RL/Storytelling_Run_MN/storytelling_dl/Neural-Net-Experiments/Spring2017-Attempt/seq2seq-Forward-Backward/checkpoints","forward_run1_epoch{}.ckpt".format(epochs))
      ...
      # print ("#########################")           
      # print ("POS: ", pos, len(train_set[bucket_id]))
      # print ("#########################")
      ...
      if epochs%10 == 0 and pos<FLAGS.batch_size:
        ...
        #        "%.2f and epochs %d" % (model.global_step.eval(), model.learning_rate.eval(),
        #                  step_time, perplexity, epochs))
        print ("global step %d learning rate %.4f step-time %.2f "
               "and epochs %d" % (model.global_step.eval(), model.learning_rate.eval(),
                         step_time, epochs))

      if epochs%save_at == 0 and pos<FLAGS.batch_size:
        # Save checkpoint and zero timer and loss.
        # checkpoint_path = os.path.join(FLAGS.train_dir, "translate.ckpt")
        checkpoint_path = os.path.join("/home/twister/Desktop/GALE_RL/Storytelling_Run_MN/storytelling_dl/Neural-Net-Experiments/Spring2017-Attempt/NewEvents-PolicyGradient/Full_Genre_68/checkpoints","forward_preload_genre68_full_run_"+str(run_num)+"_epoch{}.ckpt".format(epochs))
        model.saver.save(sess, checkpoint_path, global_step=model.global_step)
        print ("MODEL SAVED")
