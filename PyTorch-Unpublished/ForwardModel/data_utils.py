...
def hyperparam_string(config):
	"""Hyerparam string."""
	exp_name = ''
	exp_name += 'model_%s__' % (config['model']['model_name'])
	exp_name += "_"+str(datetime.datetime.now()).split(" ")[0]+"_"
	exp_name += 'hidden_%s__' % (config['model']['hidden'])
...
def pipeline_srcOnly(data, word2id, add_start=True, add_end=True):
	#data is a single story
	src_line = fillStartEnd([data], add_start, add_end)
	src_line = [[word2id[w] if w in word2id else word2id['<unk>'] for w in src_line[0]]]
	src_line = torch.LongTensor(src_line).cuda()
	return src_line
